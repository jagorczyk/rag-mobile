...
import type { Chat, FileItem, Folder, Message, Mention, Person } from './types';

const URL_KEY = 'rag.api.url';
const DEFAULT_URL = process.env.EXPO_PUBLIC_API_URL || 'http://localhost:8080';

export async function getApiUrl() { return (await AsyncStorage.getItem(URL_KEY)) || DEFAULT_URL; }
...
export async function getFiles(folderId: string) {
  const files = await request<Array<{path:string;name:string;fileType:string;extractedText?:string}>>(`/api/folders/${encodeURIComponent(folderId)}/files`);
...
export async function getChats() {
  try { const summaries = await request<Array<{id:string;name?:string;updatedAt?:string}>>('/api/chat/summaries'); return summaries.map(s => ({id:s.id,title:s.name || 'Nowa rozmowa',updatedAt:s.updatedAt})); }
  catch { const ids = await request<string[]>('/api/chat/all'); return ids.map(id => ({id, title:`Chat ${id.slice(0,8)}`, updatedAt:undefined})); }
}
export const createChat = () => request<{id:string}>('/api/chat/create');
export const getMessages = (id:string) => request<Array<{type:string;text:string;sources?:Message['sources']}>>(`/api/chat/${id}/messages`).then(items => items.map((m,i)=>({id:`${id}-${i}`,role:(m.type==='USER'?'user':'assistant') as 'user'|'assistant',content:m.text,sources:m.sources})));
