...
function Preview({ file, onClose }: { file: FileItem; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    }).catch(() => Alert.alert('BĹ‚Ä…d', 'Nie udaĹ‚o siÄ™ otworzyÄ‡ pliku.'));
  }, [file.path]);
  ...
  const choose = async (kind: 'gallery' | 'camera' | 'files') => {
    try {
      ...
      if (kind === 'gallery') {
        ...
        if (!permission.granted) return Alert.alert('Brak dostÄ™pu', 'ZezwĂłl na dostÄ™p do zdjÄ™Ä‡ w Ustawieniach.');
        ...
        if (!permission.granted) return Alert.alert('Brak dostÄ™pu', 'ZezwĂłl na dostÄ™p do aparatu w Ustawieniach.');
